...
an endless loop, it manipulates the metrics. The client discovers
dials the server, discovers metrics, and prints their values on
stdout.
...
To stop the test, hit ^C in both the terminals. The client and server
are hardwired to use TCP port 1234. If that port is already taken on
your system, then you'll have to edit the sources.

For reference, (some version of) the server and client are shown below.
Refer to the source files for the most recent version.
