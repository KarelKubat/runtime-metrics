...
The server instantiates some metrics and starts a reporter. Then, in a
...
If you don't want a server publishing metric, and a client scraping
them; but instead want a program that pushes metrics as they change,
then have a look at demosrc/publishing_program.go.

The approach is as follows: the below shown function publishMetrics()
that asks the registry for names, and using each name, gets the
appropriate metric (sum, average, etc.). The values of the metric are
returned by its Report() function. This is repeated ad infinitum with
some delay between runs. The function would typically be started as a
go-routine.
...
pretends to call some RPC that fails in about 10% of the cases. It exports
the error ratio, latency and number of calls. It can be started using
