...
If you don't want a server publishing metric, and a client scraping them; but
instead want a program that pushes metrics as they change, then have a look at
demosrc/publishing_program.go.

The approach is as follows: the below shown function publishMetrics() that asks
the registry for names, and using each name, gets the appropriate metric (sum,
